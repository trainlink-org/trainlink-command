...
const forwardStyles = computed(() => {
    return activeLoco.value.direction === Direction.forward ? 'bg-green-600' : 'bg-primary-400';
});
const reverseStyles = computed(() => {
    return activeLoco.value.direction === Direction.reverse ? 'bg-red-600' : 'bg-primary-400';
});
const stopStyles = computed(() => {
    return activeLoco.value.direction === Direction.stopped ? 'bg-primary-400 border-red-400' : 'bg-red-400 border-transparent';
});
...
<template>
    <div class="flex w-5/6 max-w-md flex-col items-center rounded-lg border-4 border-borderColor-400">
        <div class="relative my-2 flex w-11/12 items-center justify-between rounded-lg bg-primary-200 p-1"
            ...
            <ButtonComponent class="w-1/6 bg-red-400" :class="activeLoco.locked ? 'visible' : 'invisible'" @click="
                socket.emit('automation/stopAutomation', activeLoco.automationPID)
                ">
                Stop
            </ButtonComponent>
            ...
            <div class="group z-0 flex h-full w-1/6 items-center justify-end">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="justify-self-end"
                    :class="activeLoco.locked ? 'visible' : 'invisible'" viewBox="0 0 16 16">
                    <path
        ...
        <div class="rounded-lg-b h-full w-full border-borderColor-400 p-2"
            :class="activeLoco.locked ? 'bg-primary-200 opacity-40 border-t-2' : ''">
            <div class="flex w-full flex-col items-center sm:flex-row">
                ...
                <SliderComponent id="speedSlider" v-model:speed="activeLoco.speed" class="bg-inherit"
                    :disabled="throttleStatus.disabled || activeLoco.direction === Direction.stopped || activeLoco.locked" />
            </div>
            <div class="flex w-full flex-col space-y-2">
                <div class="flex w-full grow flex-row space-x-2">
                    <ButtonComponent class="basis-1/3" :class="forwardStyles"
                        :disabled="throttleStatus.disabled || activeLoco.locked" @click="
                            activeLoco.direction = Direction.forward;
                        ">
                        Forward
                    </ButtonComponent>
                    <ButtonComponent class="basis-1/3 bg-primary-400"
                        :disabled="throttleStatus.disabled || activeLoco.locked" @click="activeLoco.speed = 0">
                        Stop
                    </ButtonComponent>
                    <ButtonComponent class="basis-1/3" :class="reverseStyles"
                        :disabled="throttleStatus.disabled || activeLoco.locked"
                        @click="activeLoco.direction = Direction.reverse">
                        Reverse
                    </ButtonComponent>
                </div>
                <ButtonComponent class="border-4" :class="stopStyles"
                    :disabled="throttleStatus.disabled || activeLoco.locked"
