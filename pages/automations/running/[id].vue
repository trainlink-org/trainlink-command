...
const script = ref(runningAutomations.value.get((route.params.id as string).replace('-', '#')));

socket.on('automation/fetchRunningResponse', () => {
    script.value = runningAutomations.value.get((route.params.id as string).replace('-', '#'));
});
</script>
...
<template>
    <div class="flex h-full w-full flex-col justify-between">
        ...
        <div class="flex w-full items-center justify-around border-t-2 border-primary-300 py-1 md:hidden">
            ...
            <!-- Pause Icon -->
            <button v-if="script && script.status === AutomationStatus.running"
            ...
            <!-- Play button -->
            <button v-if="script && script.status === AutomationStatus.paused"
