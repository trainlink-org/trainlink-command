...
import { isTouchScreen, svgWidth, svgHeight, setLinkStates, DestinationState } from '../components/mapComponents/shared';
...
function setTurnout(id: number) {
    ...
    if (turnout) {
        switch (turnout.state) { //TODO refactor with an if block
...
function toggleDestination(destination: Destination) {
    if (destination !== firstDestination.value) {
        ...
        if (selectedDestinationsNum.value === 2 && firstDestination.value) {
            socket.emit('routes/setRoute', firstDestination.value.id, destination.id);
            destinationStates.set(firstDestination.value.id, DestinationState.inactive);
        ...
        <div class="h-5/6 w-11/12 overflow-y-scroll rounded-lg border-4 border-borderColor-300">
            <svg id="mapSvg" class="h-full w-full" :viewBox="viewBox">
                <ConnectorComponent v-for="link in turnoutLinks.values()" :key="link.id"
                    :start-seg-active="link.startActive" :end-seg-active="link.endActive"
                    :start="turnouts.get(link.start)?.coordinate || destinations.get(link.start)?.coordinate || { x: -5, y: -5 }"
                ...
                <DestinationComponent v-for="destination in destinations.values()" :key="destination.id"
                    :state="destinationStates.get(destination.id) || DestinationState.inactive"
