    ...
    new Promise<void>((resolve, reject) => {
        ...
        .then(() => {
            if (isNaN(locoAddress) || locoAddress <= 0 || locoAddress > 10293) {
                addError.addressMsg =
                    'Address should be a number between 1 and 10293';
...
async function editLoco() {
    ...
    new Promise<void>((resolve) => {
        ...
        .then(() => {
            if (!editError.name && !editError.address) {
                const locoAddress = Number(modalValues.address);
                socket.emit(
                    'config/editLoco',
...
<template>
    <div class="flex h-full w-full flex-col justify-between">
        <div class="overflow-y-scroll">
            <div class="flex flex-col border-b-2 border-primary-300 bg-primary-100 px-1">
                <div class="flex w-full text-lg">Throttle settings</div>
            </div>
                    ...
                    <div class="group grid w-full grid-flow-row grid-cols-5 grid-rows-2">
                        ...
                        <p class="col-span-1 row-span-2 flex flex-row space-x-3 self-center justify-self-end">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="cursor-pointer" viewBox="0 0 16 16" @click="openEditLoco(loco.address)">
                                <path
                                    d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                <path fill-rule="evenodd"
                                    d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                            </svg>
                            <input v-model="selectedItems" :value="loco.address" type="checkbox" class="cursor-pointer" />
                        </p>
        ...
        <div class="flex w-full items-center justify-around border-t-2 border-primary-300 py-1">
            ...
            <!-- Add Icon -->
            <button class="group flex h-7 w-1/6 items-center rounded-full hover:bg-primary-200 active:bg-green-400"
                @click="openAddLoco()">
                <!-- Boostrap icons: plus -->
            ...
            <!-- Delete Icon -->
            <button v-if="selectedItems.length >= 1"
    ...
    <!-- Add a locomotive -->
    <ModalComponent v-if="addOpen" title="Add a locomotive" submit-text="Add" @submit="addLoco()" @cancel="addOpen = false">
        <div class="grid h-full w-full grid-flow-row grid-cols-2 grid-rows-4 gap-y-1 sm:grid-cols-3 sm:grid-rows-2">
            <p class="col-span-2 sm:col-span-1">Name:</p>
            <div class="col-span-2">
                <input v-model="modalValues.name" type="text"
                    class="w-full rounded border-2 border-primary-400 invalid:border-red-600" :class="addError.name ? 'border-red-600' : 'border-primary-400'
                        " />
                <p :class="addError.name ? 'visible' : 'invisible'" class="text-red-600">
                    {{ addError.nameMsg }}
                </p>
            </div>
            <p class="col-span-2 sm:col-span-1">Address:</p>
            <div class="col-span-2 w-full">
                <input v-model="modalValues.address" type="number" min="1" max="10293"
                    class="w-full rounded border-2 border-primary-400 invalid:border-red-600" :class="addError.address
                        ? 'border-red-600'
                        : 'border-primary-400'
                        " />
                <p :class="addError.address ? 'visible' : 'invisible'" class="text-red-600">
                    {{ addError.addressMsg }}
                </p>
            </div>
        </div>
    ...
    <!-- Edit a locomotive -->
    <ModalComponent v-if="editOpen" title="Edit a locomotive" submit-text="Save" @cancel="editOpen = false"
        @submit="editLoco()">
        <div class="grid h-full w-full grid-flow-row grid-cols-2 grid-rows-4 gap-y-1 sm:grid-cols-3 sm:grid-rows-2">
            <p class="col-span-2 sm:col-span-1">Name:</p>
            <div class="col-span-2">
                ...
                <p class="text-red-600" :class="editError.name ? 'visible' : 'invisible'">
                    {{ editError.nameMsg }}
                </p>
            </div>
            <p class="col-span-2 sm:col-span-1">Address:</p>
            <div class="col-span-2 w-full">
                <input v-model="modalValues.address" type="number" min="1" max="10293"
                    class="w-full rounded border-2 border-primary-400 invalid:border-red-600" />
                <p class="text-red-600" :class="editError.address ? 'visible' : 'invisible'">
                    {{ editError.addressMsg }}
                </p>
            </div>
        </div>
    ...
    <!-- Delete a locomotive -->
    <ModalComponent v-if="deleteOpen" title="Confirm deletion" submit-text="Delete" @cancel="deleteOpen = false"
        @submit="deleteLoco()">
