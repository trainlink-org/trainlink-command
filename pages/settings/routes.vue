...
import { isTouchScreen, svgWidth, svgHeight, DestinationState } from '@/components/mapComponents/shared';
...
import { TurnoutState, type Coordinate, type Destination, type Turnout } from '@trainlink-org/shared-lib';
...
const points = computed(() => {
    ...
    console.log(points.map((point) => `{x: ${point.coord.x}, y: ${point.coord.y}}`).join(','));
    return points;
});

const beingMoved: Ref<number | null> = ref(null);

function drag(id: number, { offsetX, offsetY }: { offsetX: number, offsetY: number }) {
    console.log('Drag');
                ...
                turnout.connections.forEach((linkId) => {
                    ...
                    if (link) {
                        if (link.end === beingMoved.value) {
                            const offset = linkEndOffsets.get(link.id) || { x: 3, y: 3 };
                            offset.x = offsetX - dragOffset.x;
                            offset.y = offsetY - dragOffset.y;
                        } else {
                            const offset = linkStartOffsets.get(link.id) || { x: 3, y: 3 };
                            offset.x = offsetX - dragOffset.x;
                            offset.y = offsetY - dragOffset.y;
                        }
                    }
...
function dragPoint(id: number, { offsetX, offsetY }: { offsetX: number, offsetY: number }) {
    console.log('Drag');
...
function calculateCoordInverse(coord: Coordinate): Coordinate {
    return { x: calculateXCoordInverse(coord.x), y: calculateYCoordInverse(coord.y) };
}
...
function applyPointOffsets(points: Coordinate[]): Coordinate[] {
    if (points.length > 0) {
        ...
        points.forEach((point) => {
            ...
            newPoints.push(addCoords(point, calculateCoordInverse(pointOffsets.get(id) || { x: 0, y: 0 })));
        });
        return newPoints;
    } else return [];
}
...
function createLine(ID: number) {
    if (newLineStart.value !== undefined && newLineStart.value !== ID) {
        const keys = Array.from(turnoutLinks.keys()).sort((a: number, b: number) => {
            return a > b ? a : b;
        });
...
function saveChanges() {
    offsets.forEach((value, id) => {
        if (value.x !== 3 || value.y !== 3) {
            ...
            socket.emit('config/routes/changeObjectCoordinate', id, addCoords(coord, calculateCoordInverse(value)));
        }
    });
...
<template>
    <div class="flex h-full w-full flex-col">
        <svg id="mapSvg" ref="box" class="h-full w-full" :viewBox="viewBox">
            <!-- From saved map -->
            <ConnectorComponentSettings v-for="link in turnoutLinks.values()" :key="link.id"
                :start="addCoords(turnouts.get(link.start)?.coordinate || destinations.get(link.start)?.coordinate || { x: -5, y: -5 }, calculateCoordInverse(linkStartOffsets.get(link.id) || { x: 0, y: 0 }))"
                :points="applyPointOffsets(link.points)"
                :end="addCoords(turnouts.get(link.end)?.coordinate || { x: -5, y: -5 }, calculateCoordInverse(linkEndOffsets.get(link.id) || { x: 0, y: 0 }))"
                :active-route="false" />
            <TurnoutComponent v-for="turnout in turnouts.values()" :key="turnout.id" :coordinate="{
                    x: turnout.coordinate.x + calculateXCoordInverse(offsets.get(turnout.id)?.x || 0),
                    y: turnout.coordinate.y + calculateYCoordInverse(offsets.get(turnout.id)?.y || 0)
                }" :active-route="false" :title="turnout.name"
                :class="!lineEditMode ? (beingMoved === turnout.id ? 'cursor-grabbing' : 'cursor-grab') : (newLineStart === turnout.id ? 'fill-red-500' : 'fill-black')"
            ...
            <DestinationComponent v-for="destination in destinations.values()" :key="destination.id"
                :state="DestinationState.inactive" :title="destination.name"
                :coordinate="addCoords(destination.coordinate, calculateCoordInverse(offsets.get(destination.id) || { x: 0, y: 0 }))"
                :class="beingMoved === destination.id ? 'cursor-grabbing' : 'cursor-grab'"
                @mousedown="(event: MouseEvent) => drag(destination.id, event)" @mouseup="drop" />
            <ConnectorHandleComponent v-for="point in points" :key="point.id"
                :point="addCoords(point.coord, calculateCoordInverse(pointOffsets.get(point.id) || { x: 0, y: 0 }))"
                @mousedown="(event: MouseEvent) => dragPoint(point.id, event)" @mouseup="dropPoint" />

        </svg>
        <div class="relative flex w-full items-center justify-around border-t-2 border-primary-300 py-1">
            <!-- Back Icon -->
            <button
                class="group flex h-7 w-2/12 items-center rounded-full hover:bg-accent-200 active:bg-accent-400 md:hidden"
                @click="router.push('/settings')">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-8 w-full group-hover:fill-accent-400"
                    viewBox="0 0 16 16">
            ...
            <button
                class="group flex h-7 w-1/5 items-center rounded-full leading-none hover:bg-primary-200 active:bg-green-400 md:w-1/6"
                :class="!lineEditMode ? 'bg-inherit' : 'bg-accent-400'" @click="lineEditMode = !lineEditMode">
                <!-- Bootstrap icons: pencil-square -->
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="-4 -4 20 20"
                    class="h-7 group-hover:fill-green-400">
                    <path
            ...
            <button
                ...
                :class="deleteMode ? 'bg-red-400' : 'bg-inherit'" @click="deleteMode = !deleteMode">
                <!-- Bootstrap icons: trash-fill -->
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-5 group-hover:fill-red-500"
                    viewBox="0 0 16 16">
                    <path
            ...
            <button
                class="group flex h-7 w-1/5 items-center rounded-full leading-none hover:bg-primary-200 active:bg-red-400 md:w-1/6"
                :class="(changesMade ? 'visible' : 'hidden')" @click="saveChanges">
                Save changes
            </button>
            <button
                class="group flex h-7 w-1/5 items-center rounded-full leading-none hover:bg-primary-200 active:bg-red-400 md:w-1/6"
                :class="(changesMade ? 'visible' : 'hidden')" @click="discardChanges">
                Discard changes
            </button>
    ...
    <ModalComponent v-if="addModalOpen" title="Add Item" submit-text="Add" @submit="addToMap"
        @cancel="addModalOpen = false">
        <div class="flex h-20 w-full justify-around">
            <div class="flex h-full w-1/3 flex-col border border-black"
                :class="addModalSelected === AddModalType.Destination ? 'bg-accent-400' : 'bg-inherit'"
            ...
            <div class="flex h-full w-1/3 flex-col border border-black"
                :class="addModalSelected === AddModalType.Point ? 'bg-accent-400' : 'bg-inherit'"
        ...
        <div class="grid h-full w-full grid-flow-row grid-cols-2 grid-rows-4 gap-y-1 pt-5 sm:grid-cols-3 sm:grid-rows-2">
            <p class="col-span-2 sm:col-span-1">
                Name:
            </p>
            <div class="col-span-2">
                <input v-model="addModalValues.name" type="text"
                    class="w-full rounded border-2 border-primary-400 invalid:border-red-600">
                <p class="text-red-600" :class="addError.name ? 'visible' : 'invisible'">
                    {{ addError.nameMsg }}
                </p>
            </div>
            <p class="col-span-2 sm:col-span-1">
            ...
            <div class="col-span-2">
                <input v-model="addModalValues.id" type="text"
                    class="w-full rounded border-2 border-primary-400 invalid:border-red-600">
                <p class="text-red-600" :class="addError.id ? 'visible' : 'invisible'">
                    {{ addError.idMsg }}
                </p>
            </div>
        </div>
